...
export function MonthlyIncomeCard() {
  ...
  const series = [
    ...
    { name: data.realEstate.label, color: data.realEstate.color, values: data.realEstate.annualIncome.map((v) => v / 12) },
  ];
  ...
  return (
    <div className="bg-bg-2 border border-line rounded-card p-5">
      ...
      <p className="text-[11.5px] text-ink-3 mt-2">Imóvel vs Carteira · valor em R$/mês</p>
    </div>
  );
}
