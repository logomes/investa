...
const fakeSim: SimulateOut = {
  portfolio: { label: "PF", color: "#0f0", years, patrimony: portfolioPatrimony, annualIncome: years.map(() => 0), cumulativeIncome: years.map(() => 0), grossPatrimony: portfolioPatrimony, taxPaidCumulative: years.map(() => 0), exitTax: years.map(() => 0) },
  benchmark: { label: "BM", color: "#00f", years, patrimony: portfolioPatrimony.map((v) => v * 1.1), annualIncome: years.map(() => 0), cumulativeIncome: years.map(() => 0), grossPatrimony: portfolioPatrimony.map((v) => v * 1.1), taxPaidCumulative: years.map(() => 0), exitTax: years.map(() => 0) },
  sensitivity: [],
