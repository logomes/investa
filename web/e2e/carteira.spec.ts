...
test.describe("Carteira page", () => {
  ...
  test("renders allocation donut + table with the default 5 portfolio assets", async ({ page }) => {
    ...
    // Default portfolio (defaults.ts) has 4 named assets; the donut/table label them.
    await expect(page.getByText(/^FIIs$/i).first()).toBeVisible();
    await expect(page.getByText(/Ações BR Dividendos/i).first()).toBeVisible();
    await expect(page.getByText(/Dividend Aristocrats US/i).first()).toBeVisible();
